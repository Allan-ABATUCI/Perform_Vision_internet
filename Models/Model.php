...
////// -get information by id- //////
    ...
    /**
     * Obtenez la liste de compétences d'un formateur par son identifiant.
     *
    ...
    public function getCompetencesFormateurById($id_utilisateur) {
        try {
            $stmt = $this->bd->prepare("SELECT c.nom_categorie, t.nom_theme, n.libelle_niveau, aep.id_aep,c.id_categorie
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $competences[$categorie][] = array('id_aep' => $id_aep,'id_categorie'=>$id_categorie,'theme' => $theme, 'niveau' => $niveau);
            }
    ...
    public function AjtCompetenceFormateur($id_utilisateur,$nom_competence,$specialite,$niveau){
         // Récupération de l'id_theme à partir du nom de thème
        $req_id_theme=$this->bd->prepare('SELECT id_theme FROM theme WHERE nom_theme=:specialite');
        $req_id_theme->bindValue(':specialite',$specialite);
        $req_id_theme->execute();
        $id_theme=$req_id_theme->fetch();
        
        // Récupération de l'id_niveau à partir du nom du niveau
        $req_id_niveau=$this->bd->prepare('SELECT id_niveau FROM niveau WHERE libelle_niveau=:niveau');
        $req_id_niveau->bindValue(':niveau',$niveau);
        $req_id_niveau->execute();
        $id_niveau=$req_id_niveau->fetch();

       // Préparation et exécution de la requête d'insertion dans la table 'categorie'
        $requete2 = $this->bd->prepare('INSERT INTO categorie (nom_categorie, valide_categorie) VALUES (:nom_categorie, :valid_categorie)');
        $requete2->bindValue(':nom_categorie', $nom_competence);
        $requete2->bindValue(':valid_categorie', 1);
        $requete2->execute();

        // Récupération du dernier ID inséré dans la table 'categorie'
        $dernier_id_categorie = $this->bd->lastInsertId();

        // Préparation et exécution de la requête d'insertion dans la table 'aExpertiseProfessionnelle'
        $requete1 = $this->bd->prepare('INSERT INTO aExpertiseProfessionnelle (id_utilisateur, id_theme, duree_experience, id_niveau, id_categorie) VALUES (:id_utilisateur, :id_theme, :duree_experience, :id_niveau, :id_categorie)');
        $requete1->bindValue(':id_utilisateur', $id_utilisateur);
        $requete1->bindValue(':id_theme', $id_theme['id_theme']);
        $requete1->bindValue(':duree_experience', 0.00);
        $requete1->bindValue(':id_niveau', $id_niveau['id_niveau']);
        $requete1->bindValue(':id_categorie', $dernier_id_categorie);
        $requete1->execute();
    ...
    public function modif_competence($id_aep,$id_categorie,$nom_competence,$theme,$niveau){
        $req_id_theme=$this->bd->prepare('SELECT id_theme FROM theme WHERE nom_theme=:specialite');
        $req_id_theme->bindValue(':specialite',$theme);
        $req_id_theme->execute();
        $id_theme=$req_id_theme->fetch();
        ...
        $requete1=$this->bd->prepare('UPDATE aExpertiseProfessionnelle SET id_theme=:theme,id_niveau =:niveau WHERE id_aep =:id_aep');
        $requete1->bindValue(':theme',$id_theme['id_theme']);
        $requete1->bindValue(':niveau',$id_niveau['id_niveau']);
        $requete1->bindValue(':id_aep',$id_aep);
        $requete1->execute();
        
        $requete2=$this->bd->prepare('UPDATE categorie SET nom_categorie=:nom_competence WHERE id_categorie=:id_categorie');
        $requete2->bindValue(':nom_competence',$nom_competence);
        $requete2->bindValue(':id_categorie',$id_categorie);
        $requete2->execute();
    }

    public function sup_competence($id_aep,$id_categorie){
        $requete1=$this->bd->prepare('DELETE FROM aExpertiseProfessionnelle WHERE id_aep=:id_aep');
        $requete1->bindValue(':id_aep',$id_aep);
        $requete1->execute();

        $requete2=$this->bd->prepare('DELETE FROM categorie WHERE id_categorie=:id_categorie');
        $requete2->bindValue(':id_categorie',$id_categorie);
        $requete2->execute();
...
////// -pour controlleur formateurs- //////
    ...
    public function getFormateursBasicInfoByPageAndCategoryOrTheme3($page, $categoryId = null, $themeIds = null) {
        try {
            ...
            $sql = "SELECT DISTINCT u.id_utilisateur, u.nom, u.prenom, u.mail, u.photo_de_profil, f.linkedin
                    FROM utilisateur u
                    INNER JOIN formateur f ON u.id_utilisateur = f.id_utilisateur
                    INNER JOIN aExpertiseProfessionnelle aep ON f.id_utilisateur = aep.id_utilisateur";
